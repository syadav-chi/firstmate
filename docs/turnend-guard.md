...
`bin/fm-watch.sh` touches `state/.last-watcher-beat` at the top of each cycle and before every task's stale-pane scan.
Those task boundaries keep the accumulated duration of a slow fleet scan from looking like a stalled watcher, while one task read that stops making progress still lets the beacon age out.
After the last task boundary, the terminal wait (`event_wait_or_sleep`) can still last up to `FM_POLL` seconds.
